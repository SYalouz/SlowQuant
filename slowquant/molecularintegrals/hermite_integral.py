...
def R_0_0_0_0(p, XPC, YPC, ZPC, RPC, R):
    R[0,0,0,0] = boys_function_n_zero(p*RPC*RPC)
    return R[:,:,:,0]
...
def R_1_0_0_0(p, XPC, YPC, ZPC, RPC, R):
    R[0,0,0,0] = boys_function_n_zero(p*RPC*RPC)
    R[0,0,0,1] = -2.0*p * boys_function(1,p*RPC*RPC)
...
def R_1_1_1_0(p, XPC, YPC, ZPC, RPC, R):
    ...
    R[0,0,2,1] = R[0,0,0,2] + ZPC * R[0,0,1,2]
    R[0,0,3,0] = 2.0 * R[0,0,1,1] + ZPC * R[0,0,2,1]
    R[0,1,0,0] = YPC * R[0,0,0,1]
    R[0,1,1,0] = YPC * R[0,0,1,1]
    ...
    R[0,2,1,0] = R[0,0,1,1] + YPC * R[0,1,1,1]
    R[0,1,0,2] = YPC * R[0,0,0,3]
    R[0,2,0,1] = R[0,0,0,2] + YPC * R[0,1,0,2]
    R[0,3,0,0] = 2.0 * R[0,1,0,1] + YPC * R[0,2,0,1]
    R[1,0,0,0] = XPC * R[0,0,0,1]
    R[1,0,1,0] = XPC * R[0,0,1,1]
    ...
    R[2,0,1,0] = 1.0 * R[0,0,1,1] + XPC * R[1,0,1,1]
    R[1,1,0,1] = XPC * R[0,1,0,2]
    R[2,1,0,0] = 1.0 * R[0,1,0,1] + XPC * R[1,1,0,1]
    R[1,0,0,2] = XPC * R[0,0,0,3]
    R[2,0,0,1] = 1.0 * R[0,0,0,2] + XPC * R[1,0,0,2]
    R[3,0,0,0] = 2.0 * R[1,0,0,1] + XPC * R[2,0,0,1]
    return R[:,:,:,0]
...
def R_1_1_1_1(p, XPC, YPC, ZPC, RPC, R):
    ...
    R[0,0,2,2] = R[0,0,0,3] + ZPC * R[0,0,1,3]
    R[0,0,3,1] = 2.0 * R[0,0,1,2] + ZPC * R[0,0,2,2]
    R[0,0,4,0] = 3.0 * R[0,0,2,1] + ZPC * R[0,0,3,1]
    R[0,1,0,0] = YPC * R[0,0,0,1]
    R[0,1,1,0] = YPC * R[0,0,1,1]
    R[0,1,2,0] = YPC * R[0,0,2,1]
    ...
    R[0,2,2,0] = R[0,0,2,1] + YPC * R[0,1,2,1]
    R[0,1,0,2] = YPC * R[0,0,0,3]
    R[0,2,0,1] = R[0,0,0,2] + YPC * R[0,1,0,2]
    R[0,3,0,0] = 2.0 * R[0,1,0,1] + YPC * R[0,2,0,1]
    R[0,1,1,2] = YPC * R[0,0,1,3]
    R[0,2,1,1] = R[0,0,1,2] + YPC * R[0,1,1,2]
    R[0,3,1,0] = 2.0 * R[0,1,1,1] + YPC * R[0,2,1,1]
    R[0,1,0,3] = YPC * R[0,0,0,4]
    R[0,2,0,2] = R[0,0,0,3] + YPC * R[0,1,0,3]
    R[0,3,0,1] = 2.0 * R[0,1,0,2] + YPC * R[0,2,0,2]
    R[0,4,0,0] = 3.0 * R[0,2,0,1] + YPC * R[0,3,0,1]
    R[1,0,0,0] = XPC * R[0,0,0,1]
    R[1,0,1,0] = XPC * R[0,0,1,1]
    R[1,0,2,0] = XPC * R[0,0,2,1]
    ...
    R[2,0,2,0] = 1.0 * R[0,0,2,1] + XPC * R[1,0,2,1]
    R[1,1,0,1] = XPC * R[0,1,0,2]
    R[2,1,0,0] = 1.0 * R[0,1,0,1] + XPC * R[1,1,0,1]
    R[1,1,1,1] = XPC * R[0,1,1,2]
    R[2,1,1,0] = 1.0 * R[0,1,1,1] + XPC * R[1,1,1,1]
    R[1,2,0,1] = XPC * R[0,2,0,2]
    R[2,2,0,0] = 1.0 * R[0,2,0,1] + XPC * R[1,2,0,1]
    R[1,0,0,2] = XPC * R[0,0,0,3]
    R[2,0,0,1] = 1.0 * R[0,0,0,2] + XPC * R[1,0,0,2]
    R[3,0,0,0] = 2.0 * R[1,0,0,1] + XPC * R[2,0,0,1]
    R[1,0,1,2] = XPC * R[0,0,1,3]
    R[2,0,1,1] = 1.0 * R[0,0,1,2] + XPC * R[1,0,1,2]
    R[3,0,1,0] = 2.0 * R[1,0,1,1] + XPC * R[2,0,1,1]
    R[1,1,0,2] = XPC * R[0,1,0,3]
    R[2,1,0,1] = 1.0 * R[0,1,0,2] + XPC * R[1,1,0,2]
    R[3,1,0,0] = 2.0 * R[1,1,0,1] + XPC * R[2,1,0,1]
    R[1,0,0,3] = XPC * R[0,0,0,4]
    R[2,0,0,2] = 1.0 * R[0,0,0,3] + XPC * R[1,0,0,3]
    R[3,0,0,1] = 2.0 * R[1,0,0,2] + XPC * R[2,0,0,2]
    R[4,0,0,0] = 3.0 * R[2,0,0,1] + XPC * R[3,0,0,1]
    return R[:,:,:,0]
