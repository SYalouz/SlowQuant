...
def write_electron_electron(max_angular):        
    ...
    S_file.write("from slowquant.molecularintegrals.utility import Normalization, boys_function, ERI_expansion_coeff_sum, ERI_contraction\n")
    S_file.write("\n\n")
                ...
                for lc in range(max_angular+1):
                    for ld in range(max_angular+1):
                        if lc >= ld and la*(la+1)//2+lb >= lc*(lc+1)//2+ld:
                            ...
                            S_file.write("def primitive_electron_electron_"+str(la)+"_"+str(lb)+"_"+str(lc)+"_"+str(ld)+"(Coord_1_left, Coord_2_left, Coord_1_right, Coord_2_right, gauss_exp_1_left, gauss_exp_2_left, gauss_exp_1_right, gauss_exp_2_right, E_left, E_right, R, primitive):\n")
                            ...
                            S_file.write("    P_left = (gauss_exp_1_left*Coord_1_left + gauss_exp_2_left*Coord_2_left) / p_left\n")
                            S_file.write("    XAB_left = Coord_1_left - Coord_2_left\n")
                            S_file.write("    XPA_left = P_left - Coord_1_left\n")
                            S_file.write("    XPB_left = P_left - Coord_2_left\n")
                            S_file.write("    p_right = gauss_exp_1_right + gauss_exp_2_right\n")
                            S_file.write("    q_right = gauss_exp_1_right * gauss_exp_2_right / p_right\n")
                            S_file.write("    P_right = (gauss_exp_1_right*Coord_1_right + gauss_exp_2_right*Coord_2_right) / p_right\n")
                            S_file.write("    XAB_right = Coord_1_right - Coord_2_right\n")
                            S_file.write("    XPA_right = P_right - Coord_1_right\n")
                            S_file.write("    XPB_right = P_right - Coord_2_right\n")
                                                                    ...
                                                                    for y4 in range(max_angular,-1, -1):
                                                                        for z4 in range(max_angular,-1, -1):
                                                                            if x1+y1+z1 == la and x2+y2+z2 == lb and x3+y3+z3 == lc and x4+y4+z4 == ld:
                                                                                S_file.write("    primitive["+str(counter)+"] = ERI_expansion_coeff_sum(E_left["+str(x1)+","+str(x2)+",:,0],")
                                                                                ...
                                                                                counter += 1
                            S_file.write("\n")
                            S_file.write("    return 2.0*pi**(5.0/2.0)/(p_left*p_right*(p_left+p_right)**0.5)*primitive\n")
                            S_file.write("\n\n")
                ...
                for lc in range(max_angular+1):
                    for ld in range(max_angular+1):
                        if lc >= ld and la*(la+1)//2+lb >= lc*(lc+1)//2+ld:
                            ...
                            S_file.write("def electron_electron_integral_"+str(la)+"_"+str(lb)+"_"+str(lc)+"_"+str(ld)+"(Coord_1, Coord_2, Coord_3, Coord_4, gauss_exp_1, gauss_exp_2, gauss_exp_3, gauss_exp_4, Contra_coeffs_1, Contra_coeffs_2, Contra_coeffs_3, Contra_coeffs_4, primitives_buffer, Contraction_1_buffer, Contraction_2_buffer, Contraction_3_buffer, Contraction_4_buffer, E_1_buffer, E_2_buffer, R_buffer, primitives_buffer_2, output_buffer):\n")
                            S_file.write("    number_primitive_1 = len(gauss_exp_1)\n")
                            S_file.write("    number_primitive_2 = len(gauss_exp_2)\n")
                            S_file.write("    number_primitive_3 = len(gauss_exp_3)\n")
                            S_file.write("    number_primitive_4 = len(gauss_exp_4)\n")
                            ...
                            S_file.write("    Contraction_3_buffer = Contraction_3_buffer[:number_primitive_3]\n")
                            S_file.write("    Contraction_4_buffer = Contraction_4_buffer[:number_primitive_4]\n")
                            combinations = (la+1)*(lb+1)*(lc+1)*(ld+1)*(la+2)*(lb+2)*(lc+2)*(ld+2)/16
                            S_file.write("    primitives_buffer = primitives_buffer[:number_primitive_1,:number_primitive_2,:number_primitive_3,:number_primitive_4,:"+str(int(combinations))+"]\n")
                            ...
                            S_file.write("                    primitives_buffer[i,j,k,l,:] = primitive_electron_electron_"+str(la)+"_"+str(lb)+"_"+str(lc)+"_"+str(ld)+"(Coord_1, Coord_2, Coord_3, Coord_4, gauss_exp_1[i], gauss_exp_2[j], gauss_exp_3[k], gauss_exp_4[l], E_1_buffer, E_2_buffer, R_buffer, primitives_buffer_2)\n")
                            if la == 0 and lb == 0 and lc == 0 and ld == 0:
                                ...
                                S_file.write("    for i in range(0, number_primitive_2):\n")
                                S_file.write("        Contraction_2_buffer[i] = Normalization(0.0,0.0,0.0,gauss_exp_2[i]) * Contra_coeffs_2[i]\n")
                                S_file.write("    for i in range(0, number_primitive_3):\n")
                                S_file.write("        Contraction_3_buffer[i] = Normalization(0.0,0.0,0.0,gauss_exp_3[i]) * Contra_coeffs_3[i]\n")
                                S_file.write("    for i in range(0, number_primitive_4):\n")
                                S_file.write("        Contraction_4_buffer[i] = Normalization(0.0,0.0,0.0,gauss_exp_4[i]) * Contra_coeffs_4[i]\n")
                                S_file.write("    for i in range(0, "+str(int(combinations))+"):\n")
                                S_file.write("        output_buffer[i] = ERI_contraction(Contraction_1_buffer, Contraction_2_buffer, Contraction_3_buffer, Contraction_4_buffer, primitives_buffer[:,:,:,:,i])\n")
                                S_file.write("    return output_buffer\n")
                            elif la == 1 and lb == 0 and lc == 0 and ld == 0:
                                S_file.write("    for i in range(0, number_primitive_1):\n")
                                ...
                                S_file.write("        output_buffer[i] = ERI_contraction(Contraction_1_buffer, Contraction_2_buffer, Contraction_3_buffer, Contraction_4_buffer, primitives_buffer[:,:,:,:,i])\n")
                                S_file.write("    return output_buffer\n")
                            elif la == 1 and lb == 1 and lc == 0 and ld == 0:
                                S_file.write("    for i in range(0, number_primitive_1):\n")
                                S_file.write("        Contraction_1_buffer[i] = Normalization(1.0,0.0,0.0,gauss_exp_1[i]) * Contra_coeffs_1[i]\n")
                                S_file.write("    for i in range(0, number_primitive_2):\n")
                                ...
                                S_file.write("        output_buffer[i] = ERI_contraction(Contraction_1_buffer, Contraction_2_buffer, Contraction_3_buffer, Contraction_4_buffer, primitives_buffer[:,:,:,:,i])\n")
                                S_file.write("    return output_buffer\n")
                            elif la == 1 and lb == 0 and lc == 1 and ld == 0:
                                S_file.write("    for i in range(0, number_primitive_1):\n")
                                S_file.write("        Contraction_1_buffer[i] = Normalization(1.0,0.0,0.0,gauss_exp_1[i]) * Contra_coeffs_1[i]\n")
                                S_file.write("    for i in range(0, number_primitive_2):\n")
                                S_file.write("        Contraction_2_buffer[i] = Normalization(0.0,0.0,0.0,gauss_exp_2[i]) * Contra_coeffs_2[i]\n")
                                S_file.write("    for i in range(0, number_primitive_3):\n")
                                S_file.write("        Contraction_3_buffer[i] = Normalization(1.0,0.0,0.0,gauss_exp_3[i]) * Contra_coeffs_3[i]\n")
                                S_file.write("    for i in range(0, number_primitive_4):\n")
                                S_file.write("        Contraction_4_buffer[i] = Normalization(0.0,0.0,0.0,gauss_exp_4[i]) * Contra_coeffs_4[i]\n")
                                S_file.write("    for i in range(0, "+str(int(combinations))+"):\n")
                                S_file.write("        output_buffer[i] = ERI_contraction(Contraction_1_buffer, Contraction_2_buffer, Contraction_3_buffer, Contraction_4_buffer, primitives_buffer[:,:,:,:,i])\n")
                                S_file.write("    return output_buffer\n")
                            elif la == 1 and lb == 1 and lc == 1 and ld == 0:
                                S_file.write("    for i in range(0, number_primitive_1):\n")
                                S_file.write("        Contraction_1_buffer[i] = Normalization(1.0,0.0,0.0,gauss_exp_1[i]) * Contra_coeffs_1[i]\n")
                                S_file.write("    for i in range(0, number_primitive_2):\n")
                                S_file.write("        Contraction_2_buffer[i] = Normalization(1.0,0.0,0.0,gauss_exp_2[i]) * Contra_coeffs_2[i]\n")
                                ...
                                S_file.write("        Contraction_3_buffer[i] = Normalization(1.0,0.0,0.0,gauss_exp_3[i]) * Contra_coeffs_3[i]\n")
                                S_file.write("    for i in range(0, number_primitive_4):\n")
                                S_file.write("        Contraction_4_buffer[i] = Normalization(1.0,0.0,0.0,gauss_exp_4[i]) * Contra_coeffs_4[i]\n")
                                S_file.write("    for i in range(0, "+str(int(combinations))+"):\n")
                                S_file.write("        output_buffer[i] = ERI_contraction(Contraction_1_buffer, Contraction_2_buffer, Contraction_3_buffer, Contraction_4_buffer, primitives_buffer[:,:,:,:,i])\n")
                                S_file.write("    return output_buffer\n")
                            S_file.write("\n\n")
