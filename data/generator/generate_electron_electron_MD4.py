                ...
                for lc in range(max_angular+1):
                    for ld in range(max_angular+1):
                        if lc >= ld and la*(la+1)//2+lb >= lc*(lc+1)//2+ld:
                            ...
                            S_file.write("                                                                for z4 in range("+str(ld)+"-x4-y4, "+str(ld-1)+"-x4-y4, -1):\n")
                            S_file.write("                                                                    primitives_buffer[i,j,k,l,counter] = ERI_expansion_coeff_sum(E_buff_1[x1,x2,:,0],E_buff_1[y1,y2,:,1],E_buff_1[z1,z2,:,2],E_buff_2[x3,x4,:,0],E_buff_2[y3,y4,:,1],E_buff_2[z3,z4,:,2],R_array,x1+x2+1,y1+y2+1,z1+z2+1,x3+x4+1,y3+y4+1,z3+z4+1)*temp3*Norm_array[x4, y4, z4]\n")
                            S_file.write("                                                                    counter += 1\n")
                            S_file.write("                    primitives_buffer[i,j,k,l,:"+str(combinations)+"] = pi52/(p_left*p_right*(p_left+p_right)**0.5)*primitives_buffer[i,j,k,l,:"+str(combinations)+"]\n")
